...
func processRouted(data []byte, ext string, chatID int64) (*processResult, error, bool) {
    ...
    // تشخیص از روی محتوای متنی
    trimmed := strings.TrimSpace(string(data))
    if strings.HasPrefix(trimmed, "happ://") {
        if res, err := processHappContent(data); err == nil && res != nil {
            return res, nil, false
        }
    }
    if strings.HasPrefix(trimmed, "slipnet-enc://") ||
        strings.HasPrefix(trimmed, "slipnet-bundle-enc://") ||
        strings.HasPrefix(trimmed, "slipnet://") {
        return processSlipnet(data, chatID)
    }
...
// باندل‌های رمزدار SlipNet در انتظار دریافت رمز
