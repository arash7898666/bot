...
func formatQuery(q url.Values) string {
	return strings.ReplaceAll(q.Encode(), "%2F", "/")
}
...
func vlessURI(vs vnextSettingsT, ss *streamSettingsT, remarks string) (string, error) {
	...
	return fmt.Sprintf("vless://%s@%s:%d?%s#%s",
		u.Id, formatHost(v.Address), v.Port, formatQuery(q), url.PathEscape(remarks)), nil
}
...
func trojanURI(ts serversSettingsT, ss *streamSettingsT, remarks string) (string, error) {
	...
	return fmt.Sprintf("trojan://%s@%s:%d?%s#%s",
		escapeUserInfo(s.Password), formatHost(s.Address), s.Port, formatQuery(q), url.PathEscape(remarks)), nil
}
...
func outboundToURI(protocol string, settingsRaw, streamRaw json.RawMessage, remarks string) (string, error) {
	...
	case "hysteria2", "hy2":
		...
		if err := json.Unmarshal(settingsRaw, &h); err == nil && len(h.Servers) > 0 {
			...
			q := buildStreamQuery(&ss)
			return fmt.Sprintf("hysteria2://%s@%s:%d?%s#%s", escapeUserInfo(auth), formatHost(srv.Address), srv.Port, formatQuery(q), url.PathEscape(remarks)), nil
		}
		return "", fmt.Errorf("unsupported or invalid hysteria2 config")
	case "tuic":
		var t struct {
			Servers []struct {
				Address  string `json:"address"`
		...
		if err := json.Unmarshal(settingsRaw, &t); err == nil && len(t.Servers) > 0 {
			...
			q := buildStreamQuery(&ss)
			return fmt.Sprintf("tuic://%s:%s@%s:%d?%s#%s", escapeUserInfo(pass), escapeUserInfo(srv.Password), formatHost(srv.Address), srv.Port, formatQuery(q), url.PathEscape(remarks)), nil
		}
		return "", fmt.Errorf("unsupported or invalid tuic config")
	default:
...
func extractFromV2rayProfile(b []byte) ([]string, error) {
	...
	if p.Server != "" {
		// پشتیبانی کامل از Shadowsocks و Trojan از فیلدهای مستقیم
		if (p.ConfigType == 3 || p.Method != "") && p.Password != "" {
			userInfo := base64.RawURLEncoding.EncodeToString([]byte(p.Method + ":" + p.Password))
			uri := fmt.Sprintf("ss://%s@%s:%s#%s", userInfo, formatHost(p.Server), portStr, url.PathEscape(remarks))
			uris = append(uris, uri)
		} else if p.ConfigType == 4 || (p.Password != "" && p.Method == "") {
			uri := fmt.Sprintf("trojan://%s@%s:%s#%s", escapeUserInfo(p.Password), formatHost(p.Server), portStr, url.PathEscape(remarks))
			uris = append(uris, uri)
		}
	}
